...
typedef enum {
	...
	oGatewayAddress,
	oGatewayPort,
	oGatewayPath, //add for local url
	oAuthServerToken, //authserver token
	oAuthServer,
	oAuthServHostname,
	oPortalType,
	...
	oAuthServAuthAllowPathFragment,
	oHTTPDMaxConn,
	oHTTPDName,
	oHTTPDRealm,
        oHTTPDUsername,
...
static const struct {
	...
	{ "gatewaypath",		oGatewayPath }, //add for local url
	...
	{ "authallowpathfragment",	oAuthServAuthAllowPathFragment },
...
config_init(void)
{
	...
	config.gw_path = DEFAULT_GATEWAYPATH; //add for local url
...
parse_auth_server(FILE *file, const char *filename, int *linenum)
{
	...
	/* Defaults */
	authtoken = safe_strdup(DEFAULT_AUTHSERVTOKEN);
	...
	authallowpathfragment = safe_strdup(DEFAULT_AUTHSERVAUTHALLOWPATHFRAGMENT);
	...
	while ((line[0] != '\0') && (strchr(line, '}') == NULL)) {
		...
		if (strlen(p1) > 0) {
			...
			*p2 = '\0';
			...
			switch (opcode) {
				case oAuthServerToken:
					authtoken = safe_strdup(p2);
					break;
				...
				case oAuthServAuthScriptPathFragment:
					...
					break;
				case oAuthServAuthAllowPathFragment:
					free(authallowpathfragment);
					authallowpathfragment = safe_strdup(p2);
					break;
				...
				case oAuthServHTTPPort:
					http_port = atoi(p2);
					break;
				case oAuthServCMSHTTPPort:
					cmshttp_port = atoi(p2);
					break;
	...
	new->authserv_auth_allow_path_fragment = authallowpathfragment;  
	new->authserv_http_port = http_port;
	new->authserv_cmshttp_port = cmshttp_port;
	new->authserv_ssl_port = ssl_port;
	new->authserv_token = authtoken;
	
	debug(LOG_ERR, "authserv_hostname    = %s",host);
	debug(LOG_ERR, "authserv_use_ssl     = %s",ssl_available);
	debug(LOG_ERR, "authserv_path        = %s",path);
	debug(LOG_ERR, "authserv_http_port   = %d",http_port);
	debug(LOG_ERR, "authserv_cmshttp_port= %d",cmshttp_port);
	debug(LOG_ERR, "authserv_ssl_port    = %d",ssl_port);
	debug(LOG_ERR, "authserv_token       = %s",authtoken);
		...
		if (p1 && p1[0] != '\0') {
			...
			if ((strncmp(s, "#", 1)) != 0) {
				...
				case oGatewayPort:
					...
					break;
				case oGatewayPath:    
					config.gw_path= safe_strdup(p1); //add for local url
					break;
				case oPortalType:		//added by caosx 20160927
					config.portal_type = safe_strdup(p1);
					break;
				case oAuthServer:
...
/*
	funciton: gnuversion read
	linsd add for new portal 
*/
void gnuversion_read()
{
	FILE *pf;
	char line[256];

	if (pf = popen("protest --version -r | awk -F '/' '{print $3}'", "r")) {

		if ( fgets(line, 256, pf)) {
			if (line[strlen(line) - 1] == '\n')
				line[strlen(line) - 1] = '\0';

			config.gnuversion = safe_strdup(line); //gnu version 
			pclose(pf);
			return;		
		}
		pclose(pf);
	...
	config.gnuversion = safe_strdup(""); //default gnu version 
	return; 	

}

/*
	funciton: gatewaymac_read
	linsd add for new portal 
*/
void gatewaymac_read()
{
	FILE *pf;
	char line[256];

	if (pf = popen("ifconfig eth1 |grep HWaddr | awk  '{print $5}'", "r")) {

		if ( fgets(line, 256, pf)) {
			if (line[strlen(line) - 1] == '\n')
				line[strlen(line) - 1] = '\0';

			config.gw_mac= safe_strdup(line); //gateway mac 
			pclose(pf);
			return;		
		}
		pclose(pf);
	...
	config.gw_mac = safe_strdup(""); //default  
	return; 	
