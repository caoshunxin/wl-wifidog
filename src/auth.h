...
typedef struct _t_authresponse {
    ...
    char authphone[128]; /**< Authentication code returned by the server */
} t_authresponse;


/** @brief Authenticate a single client against the central server */
int authenticate_client(request *,int is_ssid_id,char *ssid_id);

/** @brief Periodically check if connections expired */
void thread_client_timeout_check(const void *arg);
void thread_weixin_timeout_check(const void *arg);
#endif
