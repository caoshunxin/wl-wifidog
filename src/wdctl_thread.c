...
static void wdctl_excess(int, char *);
...
thread_wdctl(void *arg)
{
	...
	while (1) {
		...
		if ((fd = accept(wdctl_socket_server, (struct sockaddr *)&sa_un, &len)) == -1){
			Ulog(ULOG_ERR, "Accept failed on control socket: %s\n",
					strerror(errno));
		} else {
			Ulog(ULOG_DEBUG, "Accepted connection on wdctl socket %d (%s)\n", fd, sa_un.sun_path);
			result = pthread_create(&tid, NULL, &thread_wdctl_handler, (void *)fd);
			if (result != 0) {
				Ulog(ULOG_ERR, "FATAL: Failed to create a new thread (wdctl handler) - exiting\n");
...
thread_wdctl_handler(void *arg)
{
	...
	Ulog(ULOG_DEBUG, "Entering thread_wdctl_handler....\n");
	...
	if (strncmp(request, "status", 6) == 0) {
		...
		wdctl_restart(fd);
	} else if (strncmp(request, "excess", 6) == 0) {
		wdctl_excess(fd, (request + 7));
	}

	if (!done) {
		Ulog(ULOG_ERR, "Invalid wdctl request.\n");
		shutdown(fd, 2);
		close(fd);
		pthread_exit(NULL);
	}

	Ulog(ULOG_DEBUG, "Request received: [%s]\n", request);
	
	shutdown(fd, 2);
	close(fd);
	Ulog(ULOG_DEBUG, "Exiting thread_wdctl_handler....\n");
...
wdctl_restart(int afd)
{
	...
	if (pid > 0) {
		...
		while (client) {
			/* Send this client */
			safe_asprintf(&tempstring, "CLIENT|ip=%s|mac=%s|token=%s|fw_connection_state=%u|fd=%d|counters_incoming=%llu|counters_outgoing=%llu|counters_last_updated=%lu|phone=%s|isweixin=%d|weixin_timeout=%d|type=%d|phone_type=%d\n", client->ip, client->mac, client->token, client->fw_connection_state, client->fd, client->counters.incoming, client->counters.outgoing, client->counters.last_updated,client->phone,client->isweixin,client->weixin_timeout,client->type,client->phone_type);
...
wdctl_excess(int fd, char *arg)
{
	...
	if(*str == '1'){
		debug(LOG_ERR, "当前连接人数超过额定值");
		is_excess = 1;
	}else{
		debug(LOG_ERR, "当前连接人数恢复正常");
		is_excess = 0;
	}
}
...
wdctl_reset(int fd, char *arg)
{
	...
	Ulog(ULOG_NOTICE,"[%s][%s]Exiting wdctl_reset...\n",node->mac,node->ip);
