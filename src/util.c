...
wd_gethostbyname(const char *name)
{
	...
	if(strncmp(inet_ntoa(*h_addr),"0.0.0.0",strlen("0.0.0.0"))){
		mark_online();
	}
...
get_ext_iface(void)
{
	...
	char * get_status_text() {
		...
		snprintf((buffer + len), (sizeof(buffer) - len), "Version: 20160628\n");
		len = strlen(buffer);
		...
		len = strlen(buffer);
		if (restart_orig_pid) {
			snprintf((buffer + len), (sizeof(buffer) - len), "yes\n");
			len = strlen(buffer);
		}
		else {
		...
		while (first != NULL) {
			...
			if(first->phone_type == 0){
				snprintf((buffer + len), (sizeof(buffer) - len), "  phone_type: unknow \n");
				len = strlen(buffer);
			}else if(first->phone_type == 1){
				snprintf((buffer + len), (sizeof(buffer) - len), "  phone_type: android \n");
				len = strlen(buffer);
			}else if(first->phone_type == 2){
				snprintf((buffer + len), (sizeof(buffer) - len), "  phone_type: apple \n");
				len = strlen(buffer);
			}

			snprintf((buffer + len), (sizeof(buffer) - len), "  IP: %s \n", first->ip);
			len = strlen(buffer);
			snprintf((buffer + len), (sizeof(buffer) - len), "  MAC: %s\n", first->mac);
			len = strlen(buffer);
...
//			len = strlen(buffer);
			...
			if(first->isweixin)
				snprintf((buffer + len), (sizeof(buffer) - len), "  authenticated: WX\n");
			else
				snprintf((buffer + len), (sizeof(buffer) - len), "  authenticated: %s\n", (first->fw_connection_state == 2) ? "yes":"no");
			len = strlen(buffer);

                        snprintf((buffer + len), (sizeof(buffer) - len), "  phone: %s\n", first->phone);
                        len = strlen(buffer);
...
//			len = strlen(buffer);
		...
		for (auth_server = config->auth_servers; auth_server != NULL; auth_server = auth_server->next) {
			snprintf((buffer + len), (sizeof(buffer) - len), "  Host: %s \n", auth_server->authserv_hostname);
			len = strlen(buffer);
			snprintf((buffer + len), (sizeof(buffer) - len), "        (%s)\n", auth_server->last_ip);
			len = strlen(buffer);
		}
