...
iptables_fw_init(void)
{
	...
	iptables_do_command("-t filter -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -j " TABLE_FW_MAC_WHITELIST);
	...
	execute("echo 1 > /proc/sys/net/bridge/bridge-nf-call-iptables", fw_quiet);
	...
	iptables_do_command("-t mangle -A " TABLE_WIFIDOG_TRUSTED " -m physdev --physdev-in %s -j MARK --set-mark %d", TABLE_WIFIDOG_ETHINTERFACE, FW_MARK_KNOWN);
//	iptables_do_command("-t filter -I FORWARD -p tcp --sport 443 -j ACCEPT");
//	iptables_do_command("-t filter -I FORWARD -p tcp --dport 443 -j ACCEPT");
	UNLOCK_CONFIG();
	return 1;
}
