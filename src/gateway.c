				...
				if (key && value) {
					if (strcmp(command, "CLIENT") == 0) {
						...
						if (strcmp(key, "ip") == 0) {
							...
							client->phone = safe_strdup(value);
						}
						else if (strcmp(key, "isweixin") == 0) {
							client->isweixin = atoi(value);
						}
						else if (strcmp(key, "weixin_timeout") == 0) {
							client->weixin_timeout = atoi(value);
						}
						else if (strcmp(key, "type") == 0) {
							client->type = atoi(value);
						}
						else if (strcmp(key, "phone_type") == 0) {
							client->phone_type = atoi(value);
						}
						else {
...
main_loop(void)
{
	...
	httpdAddCContent(webserver, "/wifidog", "xhauth", 0, NULL, http_callback_xhauth);
	httpdAddCContent(webserver, "/wifidog", "weixin", 0, NULL, http_callback_weixin);	

	if(strcmp(config->portal_type,"1")==0)
		Ulog(ULOG_NOTICE,"认证流程：[%s]普通认证\n",config->portal_type);
	else if(strcmp(config->portal_type,"2")==0)
		Ulog(ULOG_NOTICE,"认证流程：[%s]云白名单免认证\n",config->portal_type);
	else if(strcmp(config->portal_type,"3")==0)
		Ulog(ULOG_NOTICE,"认证流程：[%s]小鸿认证\n",config->portal_type);
	else
		Ulog(ULOG_NOTICE,"认证流程：[%s]未知\n",config->portal_type);
	
	if(config->portal_type[0] == '3')
		httpdAddC404Content(webserver, http_callback_xh404);
	else
	...
	if(strncmp(auth_server->authserv_token,"nonexistence",strlen("nonexistence"))){
		debug(LOG_ERR, "服务器连接正常: authserv_token = %s",auth_server->authserv_token);
		mark_auth_online();
	}else{
		debug(LOG_ERR, "网络异常: authserv_token = %s",auth_server->authserv_token);
		mark_offline();
	}

	/* Start weixin thread*/
	result = pthread_create(&tid_weixin, NULL, (void *)thread_weixin_timeout_check, NULL);
	if (result != 0) {
		debug(LOG_ERR, "FATAL: Failed to create a new thread (weixin) - exiting");
		termination_handler(0);
	}
	pthread_detach(tid_weixin);
	...
	while(1) {
		...
		if (webserver->lastError == -1) {
			 ...
			 * reboot the device ?
			 */
			Ulog(ULOG_ERR, "FATAL: httpdGetConnection returned unexpected value %d, exiting.\n", webserver->lastError);
			termination_handler(0);
		}
		else if (r != NULL) {
			/*
			 ...
			 */
			Ulog(ULOG_INFO, "Received connection from %s, spawning worker thread\n", r->clientAddr);
			...
			*(params + 1) = r;
			...
			if (result != 0) {
				Ulog(ULOG_ERR, "FATAL: Failed to create a new thread (httpd) - exiting\n");
				termination_handler(0);
			}else
				Ulog(ULOG_INFO, "Main to create a new thread (httpd)\n");
...
int main(int argc, char **argv) {
	...
	init_syslog();//必须在以上两个初始化完成后再初始化日志

	Ulog(ULOG_NOTICE,"---------------------------程序启动---------------------------\n");
	Ulog(ULOG_NOTICE,"---------------------------开始初始---------------------------\n");

	/* Initialize the config */
	config_read(config->configfile);
	config_validate();
	gnuversion_read(); //gnu version
	gatewaymac_read(); //gateway mac 
