...
/**check client expire time then update client list and sync fw 
**/
void
fw_client_expire_time_check(void)
{
    ...
    for (p1 = p2 = client_get_first_client(); NULL != p1; p1 = p2) {
        ...
        if (!(p1 = client_list_find(ip, mac))) {
            debug(LOG_INFO, "Node %s was freed while being re-validated!", ip);
        } else {
        	time_t	current_time=time(NULL);
            if (p1->counters.last_updated + (p1->counters.expire_time * DEFAULT_CHECKINTERVAL)
				<= current_time) {
                /* Timing out user */
                debug(LOG_INFO, "%s - Inactive for more than %ld seconds, removing client and denying in firewall",
                        p1->ip, p1->counters.expire_time * DEFAULT_CHECKINTERVAL);
...
void
fw_weixin_expire_time_check(void)
{
    t_authresponse  authresponse;
    char            *ip, *mac;
    t_client        *p1, *p2;
    int isweixin, weixin_timeout;
    ...
    for (p1 = p2 = client_get_first_client(); NULL != p1; p1 = p2) {
        	...
        	ip = safe_strdup(p1->ip);
        	mac = safe_strdup(p1->mac);
		isweixin = p1->isweixin;
	 	weixin_timeout = p1->weixin_timeout;
        	...
        	if (!(p1 = client_list_find(ip, mac))) {
        		...
        		{
				if( weixin_timeout >= 6)
				{	Ulog(ULOG_NOTICE,"[%s][%s]timeout release(weixin)\n",p1->mac,p1->ip);
			                fw_deny(p1->ip, p1->mac, FW_MARK_KNOWN);
					//p1->fw_connection_state = 0;
					served_this_session--;
                			client_list_delete(p1);		
				}
				else
				{	debug(LOG_INFO, "weixin ++");
					p1->weixin_timeout++;
				}
			}else if(p1->fw_connection_state == 0){
				if( weixin_timeout >= 45){//是否达到45*20s,连接15分钟内未认证，从链表踢除
					Ulog(ULOG_NOTICE,"[%s][%s]timeout release\n",p1->mac,p1->ip);
					client_list_delete(p1);
				}else{
					p1->weixin_timeout++;
					debug(LOG_INFO, "timeou ++,ip:%s %ds",p1->ip,p1->weixin_timeout*20);
				}
			}
        	}
...
fw_sync_with_authserver(void)
{
    ...
    for (p1 = p2 = client_get_first_client(); NULL != p1; p1 = p2) {
        ...
        if (config->auth_servers != NULL) {
            auth_server_request(&authresponse, REQUEST_TYPE_COUNTERS, ip, mac, token, incoming, outgoing, 0);
        }
	    LOCK_CLIENT_LIST();
        ...
        if (!(p1 = client_list_find(ip, mac))) {
            ...
            if (p1->counters.last_updated +
                ...
                if (config->auth_servers != NULL) {
					UNLOCK_CLIENT_LIST();
					auth_server_request(&authresponse, REQUEST_TYPE_LOGOUT, ip, mac, token, 0, 0, 0);
					LOCK_CLIENT_LIST();
                }
            } else {
                /*
